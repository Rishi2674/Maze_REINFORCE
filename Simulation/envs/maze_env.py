...
def collect_waypoints(path):
    waypoints, count = [], 0
    waypoint_interval = max(1, len(path) // 7)  # Collect 5 waypoints, or fewer if the path is short
...
class MazeEnv:
    ...
    def generate_maze(self, p):
        ...
        while internal_wall_count < n:
            ...
            if self.maze[x, y] == 0:  # Convert an empty space to a wall
                self.maze[x, y] = 1
                internal_wall_count += 1
        while internal_wall_count > n:
            x, y = internal_cells.pop()
            if self.maze[x, y] == 1:  # Convert a wall back to an empty space
                self.maze[x, y] = 0
                internal_wall_count -= 1
    ...
    def a_star(self):
        ...
        while open_set:
            ...
            if current == goal:
                ...
                if self.is_guided_maze:
                    self.waypoints, self.total_waypoints = collect_waypoints(self.path) if self.is_guided_maze else None
